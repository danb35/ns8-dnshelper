...
func TestListProvidersAndCapabilities(t *testing.T) {
	...
	for _, want := range []string{"cloudflare", "corenetworks", "desec", "digitalocean", "gandi", "godaddy", "hetzner", "linode", "namedotcom", "porkbun", "rfc2136", "route53", "vultr", "fake"} {
		if !names[want] {
...
func TestRealProvidersRejectIncompleteCredentialsBeforeAnyNetworkCall(t *testing.T) {
	for _, p := range []string{"cloudflare", "corenetworks", "desec", "digitalocean", "gandi", "godaddy", "hetzner", "linode", "namedotcom", "porkbun", "rfc2136", "route53", "vultr"} {
