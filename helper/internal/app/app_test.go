...
func TestListProvidersAndCapabilities(t *testing.T) {
	...
	for _, want := range []string{"cloudflare", "corenetworks", "desec", "digitalocean", "gandi", "godaddy", "googleclouddns", "hetzner", "ionos", "linode", "namedotcom", "porkbun", "powerdns", "rfc2136", "route53", "vultr", "fake"} {
		if !names[want] {
...
func TestRealProvidersRejectIncompleteCredentialsBeforeAnyNetworkCall(t *testing.T) {
	for _, p := range []string{"cloudflare", "corenetworks", "desec", "digitalocean", "gandi", "godaddy", "googleclouddns", "hetzner", "ionos", "linode", "namedotcom", "porkbun", "powerdns", "rfc2136", "route53", "vultr"} {
