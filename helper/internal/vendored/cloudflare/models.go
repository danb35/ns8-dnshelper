...
// Locally vendored and patched copy of github.com/libdns/cloudflare v0.2.2
// (https://github.com/libdns/cloudflare/tree/v0.2.2), copied verbatim from
...
// there fixes the bug (not yet reported upstream as of this patch).
//
// The bug: cfDNSRecord.Data.Priority/Weight/Port below are plain uint16
// with `omitempty`. encoding/json's omitempty drops a field whenever it
// holds its type's zero value -- for these fields, 0 is not "absent", it's
// the single most common real value (SRV priority 0, weight 0; a
// null/HTTPS AliasMode record's priority 0). So a perfectly normal SRV
// record like "0 0 443 target." silently loses "weight" (and "priority")
// from the JSON body sent to Cloudflare, and Cloudflare's API then answers
...
type cfDNSRecord struct {
	...
	Data       struct {
		...
		Name    string `json:"name,omitempty"`
		// PATCH: no omitempty -- see the file-level comment above. Priority
		// and weight are legitimately 0 for a great many real SRV/HTTPS
		// records, and dropping them made Cloudflare reject the request as
		// missing a required field instead of accepting the explicit 0.
		// Matches this same struct's own Tag field just below, which was
		// already unconditional for the same reason (a CAA-only field sent
		// on every record type, zero value and all, with no reported
		// problem) -- Cloudflare tolerates the extra keys on record types
		// that don't use them.
