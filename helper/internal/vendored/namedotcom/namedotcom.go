...
// Locally vendored and patched copy of github.com/libdns/namedotcom v0.9.0
// (https://github.com/libdns/namedotcom/tree/v0.9.0), copied verbatim from
// the module cache and patched only where this file says PATCH. Tracked in
// https://github.com/danb35/ns8-dnshelper/issues/19 -- drop this
// vendored copy and go back to importing the upstream module once a release
// there fixes the bug (not yet reported upstream as of this patch, and not
// independently confirmed against the live name.com API the way the
// matching Cloudflare bug was -- found by reading this code while auditing
// every provider for the same mistake after the Cloudflare bug, 2026-09-23).
//
// The bug: nameDotComRecord.Priority is a plain int32 with `omitempty`. For
// an SRV or MX record with a legitimately zero priority (SRV priority 0 is
// the common case; automx's _autodiscover._tcp record is exactly this),
// encoding/json's omitempty drops "priority" from the request body
// entirely, indistinguishable to the API from a record that never had a
// priority to begin with. Unlike weight/port here (folded into the free-text
// Answer field, so an explicit "0" survives as literal text), priority has
// its own top-level JSON field and is the one place this package hits the
// same zero-value-omitempty mistake as libdns/cloudflare's Weight/Port. See
// PATCH below (three spots: the field itself, where it's written, where
// it's read back).
...
type (
	...
	nameDotComRecord struct {
		...
		// PATCH: *int32, not int32 -- see the file-level comment above.
		// nil means "no priority field for this record type" (the omitted
		// behavior every non-SRV/MX record still gets); a non-nil 0 means
		// an explicit, real priority of zero that must reach the API.
...
func (n *nameDotComRecord) fromLibDNSRecord(id int32, record libdns.Record, zone string) {
	...
	case "SRV", "MX":
		...
		if len(fields) > 1 {
			...
			if err == nil {
				// PATCH: was `n.Priority = int32(priority)`, which is
				// indistinguishable to json.Marshal from "never set" when
				// priority is 0 -- see the file-level comment.
				p := int32(priority)
				n.Priority = &p
			}
...
func (r *nameDotComRecord) toLibDNSRecord(zone string) (libdns.Record, error) {
	...
	case "MX":
		return libdns.MX{
			Name:       name,
			TTL:        ttl,
			Preference: uint16(derefPriority(r.Priority)), // PATCH: r.Priority is now *int32
			Target:     r.Answer,
		}, nil
	case "NS":
		return libdns.NS{
	...
	case "SRV":
		...
		return libdns.SRV{
			...
			Name:      srvName,
			TTL:       ttl,
			Priority:  uint16(derefPriority(r.Priority)), // PATCH: r.Priority is now *int32
...
// derefPriority reads a possibly-nil Priority (PATCH: see the file-level
// comment), the way the un-patched code could read a plain int32 directly.
func derefPriority(p *int32) int32 {
	if p == nil {
		return 0
	}
	return *p
